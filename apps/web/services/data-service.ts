...
import type { Wish, Wishlist, User } from '../types';
...
export const wishService = new DataService<Wish>(
  () => apiClient.getWishes(),
...
export const wishlistService = new DataService<Wishlist>(
...
export const userService = new DataService<User>(
  () => apiClient.getUsers(),
  ...
  static async getUserStats() {
    try {
      ...
      const wishlists = wishlistsResponse.success
        ? (wishlistsResponse.data as Wishlist[]) || []
        : [];
      const wishes = wishesResponse.success ? (wishesResponse.data as Wish[]) || [] : [];
